...
export const MODELS = {
  FAST: 'gemini-3.1-flash-lite',   // was: gemini-2.5-flash-lite
  STANDARD: 'gemini-3.5-flash',    // was: gemini-2.5-flash
};
...
export async function runAgent({
  ...
  maxSearches = 6,
  maxTokens = 3000,
  thinkingLevel = 'low', // NEW: keep reasoning-token overhead low so budget goes to visible output
}) {
  const ai = getClient();
