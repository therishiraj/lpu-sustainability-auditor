...
// research, critique) get a shot at keeping live search on the fallback
// path — but compound is known to throw spurious "request too large"
// errors even on small prompts, so we treat it as best-effort and fall
// back further to a plain model (no search) rather than failing the step.
const GROQ_MODELS = {
...
// A 413/"request_too_large" is worth trying a smaller/simpler model for,
// even though it's not something a same-model retry would fix.
function isOversizedError(err) {
  return err?.status === 413 || /request.?too.?large/i.test(err?.message || '');
}
...
// Gemini's huge context but can trip Groq's request-size limit. Trim
// defensively on the Groq path only.
function truncateForGroq(text, maxChars = 12000) {
...
// Single, low-level Groq chat-completion call — no retry/fallback logic of
// its own, just does the HTTP round trip and normalizes the response.
async function callGroqOnce({ model, system, userContent, maxTokens }) {
...
async function runGroqAgent({ system, userContent, useWebSearch, maxTokens, tier }) {
  ...
  const primaryModel = useWebSearch ? GROQ_MODELS.SEARCH : GROQ_MODELS[tier] || GROQ_MODELS.STANDARD;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // groq/compound is known to throw spurious "request too large"
      // errors even on small prompts. Don't burn retries on the same
      // flaky model — drop straight to the plain (non-search) model
      // instead, so the step still completes, just without live search.
      if (useWebSearch && isOversizedError(err)) {
        console.warn('groq/compound rejected the request as oversized; retrying without web search...');
        try {
          return await callGroqOnce({
            model: GROQ_MODELS[tier] || GROQ_MODELS.STANDARD,
            system,
            userContent: safeUserContent,
            maxTokens: safeMaxTokens,
          });
          ...
          if (!isRetryableError(fallbackErr) || attempt === maxAttempts) throw fallbackErr;
        }
      } else if (!isRetryableError(err) || attempt === maxAttempts) {
        throw err;
      }
