...
export async function runPeerResearchAgent() {
  ...
  // Build citations from every peer institution's source_urls (cached or fresh),
  // not just live-search grounding metadata — otherwise cached runs show zero sources.
  const citationsByUrl = new Map();
  for (const entry of peer_benchmarks) {
    for (const url of entry.source_urls || []) {
      if (url && !citationsByUrl.has(url)) {
        citationsByUrl.set(url, { url, title: entry.institution });
      }
    }
  }
  const combinedCitations = [...allCitations, ...citationsByUrl.values()];
