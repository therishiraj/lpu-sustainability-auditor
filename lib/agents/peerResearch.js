...
import { getCachedPeerResearch, setCachedPeerResearch } from '@/lib/cache';
...
export async function runPeerResearchAgent(institution) {
  // Check for recent cached results first — peer sustainability data doesn't
  // change day to day, so we avoid re-running expensive live search on every audit.
  const cached = await getCachedPeerResearch(institution);
  if (cached) {
...
For each institution, note specific named programs or disclosures where you actually found them, and return the real source URLs.`;
  ...
  const result = { output, citations, searchQueries, rawText: text };

  await setCachedPeerResearch(institution, result);
